...
public:
	void mapKeys() {
		m_key[0x0] = sf::Keyboard::Num0;
		m_key[0x1] = sf::Keyboard::Num1;
		m_key[0x2] = sf::Keyboard::Num2;
		m_key[0x3] = sf::Keyboard::Num3;
		m_key[0x4] = sf::Keyboard::Num4;
		m_key[0x5] = sf::Keyboard::Num5;
		m_key[0x6] = sf::Keyboard::Num6;
		m_key[0x7] = sf::Keyboard::Num7;
		m_key[0x8] = sf::Keyboard::Num8;
		m_key[0x9] = sf::Keyboard::Num9;
		m_key[0xa] = sf::Keyboard::A;
		m_key[0xb] = sf::Keyboard::B;
		m_key[0xc] = sf::Keyboard::C;
		m_key[0xd] = sf::Keyboard::D;
		m_key[0xe] = sf::Keyboard::E;
		m_key[0xf] = sf::Keyboard::F;
	}
	void setNumSprite() {
		unsigned int pos;
		pos = ROM_SIZE - 1 - 0xf * NUM_SPR_SIZE;
	...
	unsigned short getNumSprAddr(unsigned char num) {
		num &= 0xf;
		return ROM_SIZE - 1 - 0xf * NUM_SPR_SIZE + (num * NUM_SPR_SIZE);
	}
...
public:
	...
	void exec(const Instruction& ins) {
		...
		if (ins.type == InstructionType::NONE) {
			...
			m_pc = m_stack[m_sp--]; // TODO: what if sp = 0;
		}
			...
			for (unsigned int h = 0; h < ins.byte; h++) {
				...
				for (unsigned int w = 0; w < 0x8; w++) {
					bool bit = (spr & (2 << 0x7 - w)) >> (0x7 - w) == 1;
					if (m_pixels[h][w] == 1 && bit == 0) m_V[0xf] = 1; // set collision true
					m_pixels[h][w] ^= bit;
				}
			}
			m_pc += ONE_INSTRUCTION;
		...
		// Checks the keyboard, and if the key corresponding to the value of Vx is currently in the down position, PC is increased by 2.
			if (sf::Keyboard::isKeyPressed(m_key[m_V[ins.x]]))
				m_pc += 2*ONE_INSTRUCTION;
		}
